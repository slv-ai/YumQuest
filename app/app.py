...
def main():
    print_log("Starting the YumQUEST application")
    st.title("YumQuest")
    ...
    if st.button("Ask"):
        ...
        with st.spinner("Processing..."):
            print_log(
                f"Getting answer from application using {search_type} search"
            )
            start_time = time.time()
            answer_data = get_answer(user_input, search_type)
            end_time = time.time()
...
if __name__ == "__main__":
    print_log("YumQUEST application started")
    main()